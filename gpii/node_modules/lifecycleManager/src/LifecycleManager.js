...
(function () {
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
    ...
    gpii.lifecycleManager.update = function (activeSessions, resolver, options, solutions, callback) {
        ...
        if (sessionState.noUpdatesYet) {
            sessionState.noUpdatesYet = false;
        };
    ...
    fluid.prioritizeOriginalMergePolicy = function (target, source) {
    ...
    var settingsMergePolicy = {
        "settings": fluid.prioritizeOriginalMergePolicy
    };

    gpii.lifecycleManager.applySolution = function (solution, solutionRecord, actions, sessionState) {
        var currentSnapshot = gpii.lifecycleManager.executeActions(
            solutionRecord.id, solutionRecord.settingsHandlers, actions,
            sessionState);

        var toBeMerged = [currentSnapshot[0], solution.settingsHandlers[0]];

        solution.settingsHandlers = fluid.makeArray(fluid.merge.apply(this, [settingsMergePolicy].concat(sessionState.noUpdatesYet ? toBeMerged : toBeMerged.reverse())));
    ...
    gpii.lifecycleManager.start = function (activeSessions, resolver, stop, options, solutions, callback) {
        ...
        sessionState.noUpdatesYet = true;
