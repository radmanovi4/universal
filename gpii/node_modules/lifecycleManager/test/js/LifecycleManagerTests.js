    ...
    gpii.tests.lifecycleManager.runTests = function () {
        ...
        jqUnit.test("gpii.lifecycleManager.update() with proper activeSession merging", function () {
            ...
            lifecycleManager.update(gpii.tests.lifecycleManager.userOptions, firstUpdateConfiguration, function () {
                ...
                var expectedOnFirstUpdate = {
                  "cross-hairs-clip": false,
                  "cross-hairs-color": "red"
                }

                jqUnit.assertDeepEq("update() doesn't overwrite the initial settingsHandlers, thus logging out would work properly.", expectedOnFirstUpdate, sessionSolutions[0].settingsHandlers[0].settings);
            });
            var sessionSolutions = lifecycleManager.activeSessions[gpii.tests.lifecycleManager.userOptions.userToken].solutions;
            lifecycleManager.update(gpii.tests.lifecycleManager.userOptions, secondUpdateConfiguration, function () {
                var expectedOnSecondUpdate = {
                  "cross-hairs-clip": false,
                  "cross-hairs-color": "red",
                  "mag-factor": 100
                }

                jqUnit.assertDeepEq("update() adds the initial state of a newly changed setting in solution.settingsHandlers and preserves the oldest ones for all multiply changed settings", expectedOnSecondUpdate, sessionSolutions[0].settingsHandlers[0].settings);
            });
        });
    };
}());
